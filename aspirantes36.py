...
class SistemaCorreosCompleto:
    ...
    def __init__(self):
        try:
            # Usar configuración SMTP directamente de secrets.toml
            smtp_config = {
                'smtp_server': gestor_remoto.config_completa.get('smtp_server', ''),
                'smtp_port': gestor_remoto.config_completa.get('smtp_port', 587),
                'email_user': gestor_remoto.config_completa.get('email_user', ''),
                'email_password': gestor_remoto.config_completa.get('email_password', ''),
                'notification_email': gestor_remoto.config_completa.get('notification_email', ''),
                'debug_mode': bool(gestor_remoto.config_completa.get('debug_mode', False))
            }
            
            self.smtp_server = smtp_config["smtp_server"]
            self.smtp_port = int(smtp_config["smtp_port"])
            self.email_user = smtp_config["email_user"]
            self.email_password = smtp_config["email_password"]
    ...
    def crear_sidebar(sistema_auth):
        ...
        with st.sidebar:
            ...
            if estado_sistema.estado.get('ssh_conectado'):
                ...
                st.error("❌ SSH Descon.")
                if gestor_remoto.config.get('ssh_error'):
                    st.caption(f"Error: {gestor_remoto.config['ssh_error']}")
            ...
            # Mostrar información de conexión SSH
            if gestor_remoto.config.get('host'):
                st.caption(f"🌐 Servidor: {gestor_remoto.config['host']}:{gestor_remoto.config.get('port', 22)}")
...
class SistemaInscritosCompleto:
    ...
    def __init__(self):
        ...
        logger.info("🚀 Sistema de inscritos COMPLETO (remoto) inicializado - CONFIGURACIÓN UNIFICADA")
...
class PaginaPrincipal:
    ...
    def mostrar():
        ...
        with col2:
            ...
            **💾 Backups:** {backups}
            **🌐 Servidor:** {gestor_remoto.config.get('host', 'No configurado')}
            """)
...
class PaginaConfiguracion:
    ...
    def mostrar():
        ...
        with st.expander("🌐 Gestión de Archivos Remotos", expanded=True):
            ...
            if gestor_remoto.uploads_path_remoto:
                st.caption(f"📁 Ruta base: {gestor_remoto.uploads_path_remoto}")
            if gestor_remoto.uploads_inscritos_remoto:
                st.caption(f"📁 Ruta inscritos: {gestor_remoto.uploads_inscritos_remoto}")
            if gestor_remoto.db_path_remoto:
                st.caption(f"💾 Base de datos: {gestor_remoto.db_path_remoto}")
